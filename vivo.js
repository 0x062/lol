...
// --------------------------------------------------------------------------
// 5. BRIDGE FUNCTIONS (Sama seperti sebelumnya)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// 6. RUNNER FUNCTIONS (Dengan nilai default dari data Anda, sama seperti sebelumnya)
// --------------------------------------------------------------------------
...
async function runXionToHoleskyTransfer() {
    ...
    if (!mnemonic) {
        ...
        return; // Hentikan jika mnemonic tidak ada
    }
...
async function runBabylonToXionTransfer() {
    ...
    if (!babylonRpc) {
        ...
        return; // Hentikan jika RPC Babylon tidak ada
    }
    ...
    const babylonTxAmount = process.env.BABYLON_TX_AMOUNT || "1000";
    const babylonTxDenom = process.env.BABYLON_TX_DENOM || "ubbn";
...
// --------------------------------------------------------------------------
// 7. MAIN EXECUTION LOGIC (MODIFIED)
// --------------------------------------------------------------------------
async function main() {
    const args = process.argv.slice(2);
    const transferType = args[0]; // Ambil argumen pertama dari CLI
    ...
    if (transferType === 'xion') {
    ...
    } else if (!transferType) { // Tidak ada argumen CLI
        ...
        // Beri jeda singkat antar operasi jika diperlukan (misalnya, untuk menghindari rate limit atau memberi waktu buffer laporan)
    ...
    } else { // Ada argumen, tapi tidak dikenal
        logger.warn("--------------------------------------------------------------------------------");
    ...
    await flushReport(); // Pastikan semua laporan dikirim di akhir, setelah semua operasi
